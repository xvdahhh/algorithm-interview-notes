...
(async () => {
  ...
  try {
    ...
    // Chapter 11: exchange proofs, frontier invariants and greedy boundaries.
    await open(path.join(chapters[10].dir, 'index.html'));
    assert.equal(await page.locator('.answer-list dt').count(), 32);
    assert.equal(await page.locator('a[download][href*="detail-"]').count(), 14);
    assert.equal(await page.locator('.study-table').count(), 10);
    assert.equal(await page.locator('.chapter-outline a').count(), 5);
    await page.setViewportSize({ width: 1440, height: 1000 });
    await page.screenshot({ path: path.join(out, 'chapter11-desktop.png') });
    await page.locator('#s1-before-3').scrollIntoViewIfNeeded();
    await page.screenshot({ path: path.join(out, 'chapter11-counterexamples.png') });
    const greedyExample = page.locator('.codebox').filter({
      has: page.locator('a[href="examples/detail-jump-path.cpp"]')
    });
    await greedyExample.locator('.copy').click();
    await page.waitForFunction(() => document.querySelector('.toast').textContent.includes('代码已'));
    for (const width of [390, 320]) {
      await page.setViewportSize({ width, height: 844 });
      assert.equal(await page.evaluate(() => document.documentElement.scrollWidth > innerWidth), false);
    ...
    await page.setViewportSize({ width: 390, height: 844 });
    await page.locator('#s4-before-3').scrollIntoViewIfNeeded();
    await page.screenshot({ path: path.join(out, 'chapter11-mobile-stock.png') });
    await page.locator('#s3 .answer-list').scrollIntoViewIfNeeded();
    assert.ok(await page.locator('#s3 .answer-list dd').first().isVisible());
    await page.screenshot({ path: path.join(out, 'chapter11-mobile-answers.png') });
    // Chapter 12: state semantics, dependency order and reconstruction.
    await open(path.join(chapters[11].dir, 'index.html'));
    assert.equal(await page.locator('.answer-list dt').count(), 40);
    assert.equal(await page.locator('a[download][href*="detail-"]').count(), 15);
    assert.equal(await page.locator('.study-table').count(), 12);
    assert.equal(await page.locator('.chapter-outline a').count(), 7);
    await page.setViewportSize({ width: 1440, height: 1000 });
    await page.screenshot({ path: path.join(out, 'chapter12-desktop.png') });
    await page.locator('#s2-before-2').scrollIntoViewIfNeeded();
    await page.screenshot({ path: path.join(out, 'chapter12-knapsack.png') });
    const dpExample = page.locator('.codebox').filter({
      has: page.locator('a[href="examples/detail-lis-path.cpp"]')
    });
    await dpExample.locator('.copy').click();
    await page.waitForFunction(() => document.querySelector('.toast').textContent.includes('代码已'));
    for (const width of [390, 320]) {
      await page.setViewportSize({ width, height: 844 });
      assert.equal(await page.evaluate(() => document.documentElement.scrollWidth > innerWidth), false);
    ...
    await page.waitForFunction(() => !document.querySelector('.toast').classList.contains('show'));
    await page.setViewportSize({ width: 390, height: 844 });
    await page.locator('#s3-before-3').scrollIntoViewIfNeeded();
    await page.screenshot({ path: path.join(out, 'chapter12-mobile-counting.png') });
    await page.locator('#s4 .answer-list').scrollIntoViewIfNeeded();
    assert.ok(await page.locator('#s4 .answer-list dd').first().isVisible());
    await page.screenshot({ path: path.join(out, 'chapter12-mobile-answers.png') });
    // Chapter 13: rollback contracts, prefix frequencies and lazy composition.
    await open(path.join(chapters[12].dir, 'index.html'));
    assert.equal(await page.locator('.answer-list dt').count(), 32);
    assert.equal(await page.locator('a[download][href*="detail-"]').count(), 13);
    assert.equal(await page.locator('.study-table').count(), 10);
    assert.equal(await page.locator('.chapter-outline a').count(), 5);
    await page.setViewportSize({ width: 1440, height: 1000 });
    await page.screenshot({ path: path.join(out, 'chapter13-desktop.png') });
    await page.locator('#s1-before-3').scrollIntoViewIfNeeded();
    await page.screenshot({ path: path.join(out, 'chapter13-rollback.png') });
    const structureExample = page.locator('.codebox').filter({
      has: page.locator('a[href="examples/detail-lazy-segment.cpp"]')
    });
    await structureExample.locator('.copy').click();
    await page.waitForFunction(() => document.querySelector('.toast').textContent.includes('代码已'));
    for (const width of [390, 320]) {
      await page.setViewportSize({ width, height: 844 });
      assert.equal(await page.evaluate(() => document.documentElement.scrollWidth > innerWidth), false);
    ...
    await page.waitForFunction(() => !document.querySelector('.toast').classList.contains('show'));
    await page.setViewportSize({ width: 390, height: 844 });
    await page.locator('#s4-before-4').scrollIntoViewIfNeeded();
    await page.screenshot({ path: path.join(out, 'chapter13-mobile-tags.png') });
    await page.locator('#s3 .answer-list').scrollIntoViewIfNeeded();
    assert.ok(await page.locator('#s3 .answer-list dd').first().isVisible());
    await page.screenshot({ path: path.join(out, 'chapter13-mobile-answers.png') });
    // Chapter 14: cross-structure invariants, grouped events and sampling.
    await open(path.join(chapters[13].dir, 'index.html'));
    assert.equal(await page.locator('.answer-list dt').count(), 32);
    assert.equal(await page.locator('a[download][href*="detail-"]').count(), 13);
    assert.equal(await page.locator('.study-table').count(), 10);
    assert.equal(await page.locator('.chapter-outline a').count(), 5);
    await page.setViewportSize({ width: 1440, height: 1000 });
    await page.screenshot({ path: path.join(out, 'chapter14-desktop.png') });
    await page.locator('#s2-before-3').scrollIntoViewIfNeeded();
    await page.screenshot({ path: path.join(out, 'chapter14-events.png') });
    const combinedExample = page.locator('.codebox').filter({
      has: page.locator('a[href="examples/detail-lfu-cache.cpp"]')
    });
    await combinedExample.locator('.copy').click();
    await page.waitForFunction(() => document.querySelector('.toast').textContent.includes('代码已'));
    for (const width of [390, 320]) {
      await page.setViewportSize({ width, height: 844 });
      assert.equal(await page.evaluate(() => document.documentElement.scrollWidth > innerWidth), false);
    ...
    await page.setViewportSize({ width: 390, height: 844 });
    await page.locator('#s3-before-3').scrollIntoViewIfNeeded();
    await page.screenshot({ path: path.join(out, 'chapter14-mobile-random.png') });
    await page.locator('#s4 .answer-list').scrollIntoViewIfNeeded();
    assert.ok(await page.locator('#s4 .answer-list dd').first().isVisible());
    await page.screenshot({ path: path.join(out, 'chapter14-mobile-answers.png') });
    // Chapter 15: interview contracts, executable tests and study plans.
    await open(path.join(chapters[14].dir, 'index.html'));
    assert.equal(await page.locator('.answer-list dt').count(), 32);
    assert.equal(await page.locator('a[download][href*="detail-"]').count(), 8);
    assert.equal(await page.locator('.study-table').count(), 10);
    assert.equal(await page.locator('.chapter-outline a').count(), 5);
    await page.setViewportSize({ width: 1440, height: 1000 });
    await page.screenshot({ path: path.join(out, 'chapter15-desktop.png') });
    await page.locator('#s2-before-2').scrollIntoViewIfNeeded();
    await page.screenshot({ path: path.join(out, 'chapter15-testing.png') });
    const interviewExample = page.locator('.codebox').filter({
      has: page.locator('a[href="examples/detail-prefix-count.cpp"]')
    });
    await interviewExample.locator('.copy').click();
    await page.waitForFunction(() => document.querySelector('.toast').textContent.includes('代码已'));
    for (const width of [390, 320]) {
      await page.setViewportSize({ width, height: 844 });
      assert.equal(await page.evaluate(() => document.documentElement.scrollWidth > innerWidth), false);
    ...
    await page.setViewportSize({ width: 390, height: 844 });
    await page.locator('#s4-before-2').scrollIntoViewIfNeeded();
    await page.screenshot({ path: path.join(out, 'chapter15-mobile-plan.png') });
    await page.locator('#s3 .answer-list').scrollIntoViewIfNeeded();
    assert.ok(await page.locator('#s3 .answer-list dd').first().isVisible());
    await page.screenshot({ path: path.join(out, 'chapter15-mobile-answers.png') });
