...
<h3>知识与适用场景</h3><p>{esc(concept)}</p><div class="invariant"><span>推导 / 不变量</span><p>{esc(reason)}</p></div>
...
<div class="practice"><h3>经典考点与练习</h3><p>{esc(practice)}</p><details><summary>面试官可能继续问什么？</summary><p>{esc(question)}</p><p class="muted">先用自己的话回答，再回到本节的前提、不变量和边界检查论证。</p></details></div>
...
<section class="reading-guide"><strong>学习顺序与使用约定</strong><p>第 1–6 章打基础，第 7–10 章学习树与搜索，第 11–12 章攻克贪心和动态规划，第 13–14 章按需进阶，第 15 章安排训练与模拟。页面中的掌握勾选会在浏览器允许时保存在本地；它记录自评，不代表题目已通过测试。</p><p>所有文件均可离线阅读。C++ 示例按小节独立提供，点击“下载 .cpp”可获得带标准头文件的源文件。</p></section>'''
    ...
    print(f'Generated {len(CHAPTERS)+1} HTML pages and {sum(len(c["sections"]) for c in CHAPTERS)} C++ examples.')
